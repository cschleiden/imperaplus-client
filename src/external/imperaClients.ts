...
export class AccountClient {
    ...
    getManageInfo(returnUrl: string | null, generateState: boolean): Promise<ManageInfoViewModel | null> {
...
export class GameClient {
     ...
     */
    postJoin(gameId: number): Promise<FileResponse | null> {
...
export class TournamentClient {
     ...
     */
    postCreateTeam(tournamentId: string, name: string | null, password: string | null): Promise<TournamentTeam | null> {
        let url_ = this.baseUrl + "/api/tournaments/{tournamentId}/teams?";
        if (tournamentId === undefined || tournamentId === null)
            throw new Error("The parameter 'tournamentId' must be defined.");
        url_ = url_.replace("{tournamentId}", encodeURIComponent("" + tournamentId)); 
     ...
     */
    postJoinTeam(tournamentId: string, teamId: string, password: string | null): Promise<TournamentTeam | null> {
...
export class AllianceClient {
     ...
     */
    post(creationOptions: AllianceCreationOptions | null): Promise<AllianceSummary | null> {
        let url_ = this.baseUrl + "/api/alliances";
        url_ = url_.replace(/[?&]$/, "");
    ...
    protected processPost(response: Response): Promise<AllianceSummary | null> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v, k) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
