...
export interface IPromiseAction<TResult, TData>
    extends IAction<IAsyncPayload<TResult, TData>> {
    options?: IApiActionOptions;
}

export const success = (type: string, suffix?: string) =>
    `${type}-success${(suffix && `-${suffix}`) || ""}`;
export const failed = (type: string, suffix?: string) =>
    `${type}-failed${(suffix && `-${suffix}`) || ""}`;
export const pending = (type: string, suffix?: string) =>
    `${type}-pending${(suffix && `-${suffix}`) || ""}`;
...
export const makePromiseAction = <TInput, TResult>(
    ...
    action: (
        ...
        getState?: () => IState,
        deps?: IAsyncActionDependencies
    ) => IMadePromiseAction<TResult, TInput>
): IPromiseActionCreator<TInput, TResult> => {
