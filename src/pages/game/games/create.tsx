...
const CreateGameComponent: AppNextPage<{ maps: MapTemplateDescriptor[] }> = (
    props
) => {
    const maps = props.maps.filter((m: MapTemplateDescriptor) => m.isActive);
        ...
        <GridRow>
            <GridColumn className="col-md-8">
                <p>
                    <LinkString
                        link={__(
                            "Here you can create a new fun game. If you want to start playing sooner, you might want to [join](/game/games/join) an existing game. Fun games do not count for the ladder."
                        )}
                    />
                </p>
                ...
                <Form
                    ...
                    onSubmit={async (formState, dispatch, getState, extra) => {
                        ...
                        await create(dispatch, getState, extra, {
                            ...
                            newUnitsPerTurn: formState.getFieldValue(
                                "unitsperturn",
                                3
                            ),
                            victoryConditions: [
                            ...
                            initialCountryUnits: formState.getFieldValue(
                                "initialCountryUnits",
                                1
                            ),
                            maximumNumberOfCards: formState.getFieldValue(
                                "maximumNumberOfCards",
                                5
                            ),
                            minUnitsPerCountry: formState.getFieldValue(
                                "minUnitsPerCountry",
                                1
                            ),
                            maximumTimeoutsPerPlayer: formState.getFieldValue(
                                ...
                                <Tab eventKey={1} title={__("Simple")}>
                                    <GridRow>
                                        ...
                                        <GridColumn className="hidden-xs hidden-sm col-md-5">
                                            {formState.getFieldValue("map") && (
                                                <MapPreview
                                                    mapTemplateName={formState.getFieldValue(
                                                        "map"
                                                    )}
                                                    responsive={true}
                                                />
                                            ...
                                            {formState.getFieldValue("map") && (
                                                <Link
                                                    ...
                                                    as={`/game/mappreview/${formState.getFieldValue(
                                                        "map"
                                                    )}`}
                                                >
                                                    <a>{__("Preview")}</a>
                                ...
                                <Tab eventKey={2} title={__("Advanced")}>
                                    <GridRow>
                                        <GridColumn className="col-md-6">
                                            ...
                                            <ControlledTextField
                                                label={__(
                                                    "Minimum units per country"
                                                )}
                                                placeholder={__(
                                                    "Minimium units per country"
                                                )}
                                                type="number"
                                        ...
                                        <GridColumn className="col-md-6">
                                            ...
                                            >
                                                ...
                                                >
                                                    {__("Everything visible")}
                                                </option>
                                                <option
                                                    key={
                                                        VisibilityModifierType.Fog
                                                    }
                                                    value={
                                                        VisibilityModifierType.Fog
                                                    }
            ...
            <GridColumn className="col-md-4">
                ...
                <p>
                    <LinkString
                        link={__(
                            "Here you can create a new game. You can also [join](/game/games/join) an existing game, which might be faster."
                        )}
                    />
                </p>
                ...
                <p>
                    {__(
                        "Simple game settings group the most common settings, if you are not familiar with the other settings, just use the default values."
                    )}
                </p>
                ...
                <p>
                    {__(
                        "These settings allow you to customize nearly every aspect of the created game."
                    )}
                </p>
            </GridColumn>
        </GridRow>
...
CreateGameComponent.getInitialProps = async (ctx) => {
    await ctx.store.dispatch(getMaps());

    // await ctx.store.dispatch(fetch());

    return {
        maps: ctx.store.getState().general.lookup["maps"],
    };
};

CreateGameComponent.getTitle = () => "Create Fun Game";
CreateGameComponent.needsLogin = true;
